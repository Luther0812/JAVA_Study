...
public class ModifyTree_016 extends JApplet{
	private JComboBox jcboSelectionMode = new JComboBox(new String[]{"CONTIGOUS_TREE_SELECTION", "DISCONTIGUOUS_TREE_SELECTION", "SINGLE_TREE_SELECTION"});
	...
	public ModifyTree_016(){
		...
		p2.add(new JScrollPane(jTree1 = new JTree(new DefaultTreeModel(root))));
		...
		jbtRemove.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent e){
				...
				for(int i = 0; i < paths.length; i++){
					...
					if(node.isRoot()){
						JOptionPane.showMessageDialog(null, "Can not remove the root");
					}else{
